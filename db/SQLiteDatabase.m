...
%   > import edu.stanford.covert.db.SQLiteDatabase;
%   > db = SQLiteDatabase(hostName, schema, userName, password);
...
classdef SQLiteDatabase < Database
    properties (SetAccess = protected)
        dbConn                 %The database connection
        pathToDB               %The database host
    ...
    methods
        ...
        function prepareStatement(this, sql, varargin)
            ...
            for i = 1:length(startIndex)
                switch tokenStr{i}{1}
                    ...
                    case 'M',     throw(MException('SQLiteDatabase:unsupportedType', tokenStr{i}{1}));
                    case 'B',     throw(MException('SQLiteDatabase:unsupportedType', tokenStr{i}{1}));
                    case {'F', 'uF'},
        ...
        function [out, columnNames] = query(this)
            ...
            try
                ...
                throw(MException('SQLiteDatabase:mysqlError', ...
            ...
            for i = 1:metaData.getColumnCount()                      
                ...
                switch columnTypes{i}{1}
                    ...
                    case 'FIELD_TYPE_NEWDATE',     throw(MException('SQLiteDatabase:unsupportedType', columnTypes{i}{1}))
                    ...
                    case 'FIELD_TYPE_GEOMETRY',    throw(MException('SQLiteDatabase:unsupportedType', columnTypes{i}{1}))
                    ...
                    switch columnTypes{i}{1}
                        ...
                        case 'FIELD_TYPE_LONGLONG',
                            ...
                            if numel(absByteArray) > 8
                                ...
                                if absByteArray(1) ~= 0
                                    throw(MException('SQLiteDatabase:unexpectedOverflow', char(resultSet.getBigDecimal(i))));
                                end
                        ...
                        case {'FIELD_TYPE_GEOMETRY', 'FIELD_TYPE_NEWDATE'}
                            throw(MException('SQLiteDatabase:unsupportedType', columnTypes{i}{1}))
                        otherwise
                            this.data.(fieldName)(j, 1) = double(resultSet.getDouble(i));
                            if resultSet.wasNull()
                                this.data.(fieldName)(j, 1) = double(this.nullValue);
    ...
    methods (Access = protected)        
        function open(this)
            try
                ...
                url = ['jdbc:sqlite:' this.pathToDB];
                this.dbConn = driver.connect(url, properties);
            catch exception
                throw(MException('SQLiteDatabase:mysqlError', char(exception.message)));
            end
        end
